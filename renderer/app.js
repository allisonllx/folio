...
const relationships = window.folioRelationships.labels;
const sourceLabel = skill => skill.source === 'Personal' ? 'Local folder' : skill.source==='Plugin'?'Plugin cache':skill.source || 'Local folder';
let relationshipFilter = 'all';
let sourceMode = 'rendered';
...
const relationship = skill => window.folioRelationships.effective(skill,meta(skill));
...
async function load(refresh=false,quiet=false) {
  ...
  try {
    ...
    render();
    lastScan=Date.now();
    if(refresh&&!quiet) toast('Your shelves are up to date.');
  } catch(error) {
...
function matching() {
  return data.skills.filter(skill => (view==='all'||view==='favorites'&&meta(skill).favorite||view==='reviews'&&meta(skill).reviews?.length||category(skill)===view) && (relationshipFilter==='all'||relationship(skill)===relationshipFilter) && (!query||`${skill.name} ${skill.description} ${category(skill)} ${meta(skill).notes||''}`.toLowerCase().includes(query)));
}
function render() {
  ...
  const list=matching();
  $('#result-count').textContent=`${list.length} ${list.length===1?'skill':'skills'}${relationshipFilter!=='all'?` · ${relationships[relationshipFilter]}`:query?' matching your search':view==='all'?` · ${cats.length} shelves`:''}`;
  $('#surprise-button').disabled=!list.length;
  if(!list.length) { $('#shelves').innerHTML=`<div class="empty"><span>▤</span><h2>No books on this shelf yet.</h2><p>${relationshipFilter!=='all'?'No skills match this type here. Choose All skill types to see the rest.':query?'Try another word, or clear your search.':view==='favorites'?'Open a skill and tap the heart to bookmark it.':view==='reviews'?'Open a skill and add your first run review.':'Install a skill in one of the folders listed in Library details, then refresh.'}</p></div>`;return; }
  ...
  $('#shelves').innerHTML=cats.filter(cat=>list.some(s=>category(s)===cat)).map(cat=>{
    const all=list.filter(s=>category(s)===cat).sort((a,b)=>(a.source==='Personal'?0:1)-(b.source==='Personal'?0:1)||a.name.localeCompare(b.name)); const books=view==='all'&&!query&&relationshipFilter==='all'?all.slice(0,capacity):all;
    return `<section class="shelf-section"><div class="shelf-heading"><h2>${esc(cat)}</h2><span>${all.length}</span><small>${esc(shelfNotes[cat]||'room for discovery')}</small>${books.length<all.length?`<button class="see-shelf" data-shelf="${esc(cat)}">View all ${all.length} ↗</button>`:''}</div><div class="book-row">${books.map((s,i)=>`<button class="book" data-id="${s.id}" style="--book-color:${color(s,i)}" aria-label="${esc(title(s))}">${meta(s).favorite?'<span class="book-mark"></span>':''}<span class="book-top">${esc(sourceLabel(s))}<span class="book-star">${['✧','⁕','⌁','✳'][i%4]}</span></span><span class="book-title">${esc(title(s))}</span><span class="book-desc">${esc(s.description||'Open to explore this skill’s instructions.')}</span><span class="book-bottom"><span>${s.provenance?.status==='changed'?'Review changes · ':''}${esc(relationships[relationship(s)])}${meta(s).reviews?.length?` · ${meta(s).reviews.length} review${meta(s).reviews.length===1?'':'s'}`:''}</span><span>↗</span></span></button>`).join('')}</div></section>`;
  }).join('');
...
function renderDetail() {
  ...
  if(tab==='source'){
    document.querySelectorAll('[data-source-mode]').forEach(b=>b.onclick=()=>{sourceMode=b.dataset.sourceMode;renderDetail();document.querySelector(`[data-source-mode="${sourceMode}"]`).focus();});
    document.querySelectorAll('[data-copy-link]').forEach(b=>b.onclick=async()=>{try{await window.folio.copy(b.dataset.copyLink);toast('Link address copied.');}catch(e){toast(e.message);}});
  }
  if(tab==='overview'){
    $('#skill-relationship').onchange=async e=>{
      const field=e.target, previous=meta(s).relationship||'auto';
      field.disabled=true;
      // Preserve any unsaved margin notes: only the shelves need rerendering.
      if(!await save({relationship:field.value},'Skill type saved.'))field.value=previous;
      else $('#provenance-evidence').innerHTML=provenanceContent(s);
...
function provenanceContent(s) {
  const p=s.provenance||{},manual=meta(s).relationship&&meta(s).relationship!=='auto';
  const heading=p.status==='verified'?'Verified against original':p.status==='changed'?'Review changes':'Not yet verified';
  const activation=p.kind==='plugin'?`<p>Plugin setting: ${p.activation==='enabled'?'enabled in local Codex config':p.activation==='disabled'?'disabled in local Codex config':'not established from local config'}. This does not confirm that this cached version is active in your current task.</p>`:'';
  return `<section class="evidence ${p.status==='changed'?'changed':''}"><strong>${heading}</strong><p>${esc(p.reason||'No installation evidence found.')}</p>${manual?'<p class="manual-note">Your manual label takes precedence.</p>':''}${p.suggestion==='adapted'?'<p>If these are your edits, choose <b>Adapted by me</b> above.</p>':''}${activation}${p.source?`<p>Source: <b>${esc(p.source)}</b>${p.version?` · ${esc(p.version)}`:''}</p>`:''}<details><summary>Evidence details</summary>${p.recordPath?`<p>Record: <code>${esc(p.recordPath)}</code></p>`:''}${p.sourceUrl?`<p>Repository: <code>${esc(p.sourceUrl)}</code></p>`:''}${p.baselinePath?`<p>Bundled original: <code>${esc(p.baselinePath)}</code></p>`:''}<p>Current folder: <code>${esc(s.fingerprint?.gitTree||'Unavailable')}</code></p>${p.expectedHash?`<p>Original folder: <code>${esc(p.expectedHash)}</code></p>`:''}<p>Includes file contents, paths, executable bits and symlink definitions. External symlink targets and Git metadata aren’t compared. No files are uploaded.</p></details></section>`;
}
function overview(s,m) {return `<div class="detail-columns"><section><h3>A place on your shelf</h3><label for="skill-relationship">My relationship to this skill</label><select id="skill-relationship"><option value="auto" ${!m.relationship||m.relationship==='auto'?'selected':''}>Automatic · ${esc(relationships[s.provenance?.autoRelationship||'unclassified'])}</option>${Object.entries(relationships).map(([value,label])=>`<option value="${value}" ${m.relationship===value?'selected':''}>${esc(label)}</option>`).join('')}</select><p class="hint">Created: you wrote it, including with an agent. Adapted: you changed someone else’s skill. Installed: you kept the online original. Choose Automatic to follow verified installation evidence.</p><div id="provenance-evidence">${provenanceContent(s)}</div><label for="skill-category">Shelf</label><select id="skill-category">${Object.keys(colors).map(c=>`<option ${category(s)===c?'selected':''}>${esc(c)}</option>`).join('')}</select><p class="hint">Suggested from the skill’s name and description. Move it wherever it makes sense to you.</p><label>Start with a clear task</label><div class="prompt-preview">${esc(`Use ${s.name} to help me…\n\n[Your task, inputs and desired result]`)}</div><p class="hint">The copied prompt includes the exact local skill path. It works on this Mac.</p></section><section><h3>Your margin notes</h3><label for="notes">Personal notes</label><textarea id="notes" rows="8" maxlength="20000" placeholder="What is this useful for? Anything you want to remember before using it?">${esc(m.notes||'')}</textarea><div class="form-footer"><span>Private to this library</span><button class="primary" id="save-notes">Save notes</button></div><p class="hint">Notes stay separate from the original skill instructions.</p></section></div>`;}
function source(s) {return `<h3>The original instructions</h3><p>Read directly from your installed skill. Folio never runs or edits these instructions.</p><div class="source-modes" role="group" aria-label="Instruction display"><button class="secondary" data-source-mode="rendered" aria-pressed="${sourceMode==='rendered'}">Rendered Markdown</button><button class="secondary" data-source-mode="raw" aria-pressed="${sourceMode==='raw'}">Raw source</button></div>${sourceMode==='raw'?`<pre class="source-pre">${esc(s.body)}</pre>`:`<p class="hint">Links copy their address. Images are shown as references.</p><article class="markdown-body">${window.folioInstructions.render(s.body)}</article>`}<label>Installed ${s.locations?.length>1?'locations':'location'}</label><div class="source-path">${(s.locations?.length?s.locations:[s.path]).map(esc).join('<br>')}</div>${s.locations?.length>1?'<p class="hint">Identical copies share this book. Different contents remain separate books.</p>':''}`;}
...
$('#relationship-filter').innerHTML='<option value="all">All skill types</option>'+Object.entries(relationships).map(([value,label])=>`<option value="${value}">${esc(label)}</option>`).join('');
$('#relationship-filter').onchange=e=>{relationshipFilter=e.target.value;render();};
...
$('.brand').onclick=e=>{e.preventDefault();view='all';query='';relationshipFilter='all';$('#relationship-filter').value='all';$('#search').value='';render();window.scrollTo(0,0);};
...
load();
window.addEventListener('focus',()=>{if(Date.now()-lastScan>10000&&!document.querySelector('dialog[open]'))load(true,true);});
